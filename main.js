...
function getPlayerInput() {
  return prompt(
    `Please input your move ${playerName} - 'rock', 'paper' or 'scissors'`
  );
}
...
function getPlayerName() {
  ...
  playerName = prompt(
    "Please input your User Name. This should be no more than 10 characters and must start with a letter"
  );

  while (validName === false) {
    while (checkFirstLetter() === false) {
      playerName = prompt(
        `Sorry that is not a valid User Name! The first character of your User Name MUST BE A LETTER!\n\nPlease enter a valid user name`
      );
    }
    while (checkNameLength() === false) {
      playerName = prompt(
        `Sorry that is not a valid User Name! Please input a User Name LESS THAN 10 CHARACTERS!\n\nPlease enter a valid user name`
      );
    }
